...
class MyVector{
    ...
    MyVector (int size=0, T defaultValue= T()): m_size(size), m_defaultValue(defaultValue){
        ...
        for (int i{0}; i<m_size; i++){
            m_vector[i]= new T(m_defaultValue);
        }
    }
    T& operator[] (int i){
        return *m_vector[i];
    ...
    void PushBack (T NewValue){
         ...
         tmpVector[m_size] = new T(NewValue); // Allocate memory for the new element
    ...
    void resize (int NewSize, T NewValue= T()){
        if (NewSize>m_size){
            ...
            for (int k{m_size}; k< NewSize; k++){
                tmpVector[k] = new T(NewValue); // Allocate memory for new elements
            }
    ...
    void insert (int i, T insertedValue){
        ...
        tmpVector[i] = new T(insertedValue); // Allocate memory for the inserted value
    ...
    ~MyVector() {
        for (int i=0; i<m_size; i++) delete m_vector[i];
        delete [] m_vector;
    }
};
class shape{
    public:
    int* m_data= new int [100*1024*1024]; //Allocating a fairly large memory for the “data” member
    ~shape() { 
    delete[] m_data; 
}
